...
public class Banstatus implements CommandExecutor{
	
	private hBans plugin = hBans.getInstance();
	private MySQL db = plugin.db;
	@Override
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if(cmd.getName().equalsIgnoreCase("banstatus"))
		{
			if(args.length!=1)
			{
			...
			try{
				...
				query = "SELECT * FROM " + plugin.BansTable + " WHERE UUID = \"" + uuidString + "\";";
				rs = db.querySQL(query);
				if(!rs.first())
				{
					sender.sendMessage(Message.playerNotBanned(args[0]));
					return true;
				}
				Timestamp time = rs.getTimestamp("Time");
				if(System.currentTimeMillis() > time.getTime() && time.getTime()!=0)
				{
					query = "DELETE FROM "+plugin.BansTable+" WHERE UUID = \"" + uuidString + "\";";
					db.updateSQL(query);
					sender.sendMessage(Message.playerNotBanned(args[0]));
					return true;
				}
